...
class XLSXDataTableExpandDataMixin:
    ...
    def expand_data(self):
        ...
        for parent_id, data_lists in parent_to_data_lists.items():
            parent_data = dict(region_id=parent_id) | self.get_aggr_data(
                data_lists
            )
            expanded_data_list.append(parent_data)

        expanded_data_list.sort(key=lambda d: d["region_id"])

        os.makedirs(self.dir_table, exist_ok=True)
        json_file.write(expanded_data_list)
        log.info(f"Wrote {len(expanded_data_list)} rows to {json_file}")
        ...
        gnd_ids_in_data_table = set(d["gnd_id"] for d in data_list)
